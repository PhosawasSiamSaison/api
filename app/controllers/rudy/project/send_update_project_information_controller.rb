...
class Rudy::Project::SendUpdateProjectInformationController < Rudy::ApplicationController
  def call
    # return render_demo_response if get_demo_bearer_token?

    tax_id            = params[:tax_id]
    site_code         = params[:project_code]
    new_site_code     = params[:new_project_code]
    ...
    error, result = update_site_error_check(false, is_project: true)
    ...
    contractor.contractor_users.each do |contractor_user|
      SendMessage.send_update_project_information(contractor_user, site_information)
    end
